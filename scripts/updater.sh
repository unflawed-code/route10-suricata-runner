...
# from the user backup is applied on top so the user's settings win.
merge_policy_conf() {
    local new_conf="$1"
    local user_bak="$2"
    [ -f "$new_conf" ] || return 0
    ...
    local merged="${new_conf}.merged"
    cp "$new_conf" "$merged"

    grep -v '^[[:space:]]*#' "$user_bak" | grep '=' | while IFS='=' read -r key value; do
        key=$(echo "$key" | tr -d ' \n\r\t')
        [ -z "$key" ] && continue
        if grep -q "^${key}=" "$merged"; then
            sed -i "s|^${key}=.*|${key}=${value}|" "$merged"
        else
    ...
    mv "$merged" "$new_conf"
    # Sync back to REMOTE_DIR copy if the live conf lives elsewhere
    [ "$new_conf" != "${REMOTE_DIR}/ips-policy.conf" ] && \
        cp -f "$new_conf" "${REMOTE_DIR}/ips-policy.conf"
    log "ips-policy.conf merged: new defaults preserved, user values applied on top."
}
...
perform_update() {
    ...
    # --- Stage 1: Back up user config before the bulk overwrite ---
    local user_conf_bak="${UPDATE_TMP_DIR}/ips-policy.conf.user"
    cp "$POLICY_CONF" "$user_conf_bak" 2>/dev/null || true

    # --- Stage 2: Apply the update (overwrites repo files) ---
